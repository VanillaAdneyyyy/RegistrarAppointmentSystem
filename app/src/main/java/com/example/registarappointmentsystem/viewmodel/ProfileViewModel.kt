...
import com.example.registarappointmentsystem.data.repository.ProfileRepositoryImpl
    ...
    fun loadProfile(userId: Int) {
        ...
        viewModelScope.launch {
            try {
                ...
                _profileState.value = UiResult(errorMessage = e.message ?: "Failed to load profile")
            }
        }
    }
}
...
class ProfileViewModelFactory : ViewModelProvider.Factory {
    ...
    override fun <T : ViewModel> create(modelClass: Class<T>): T {
        if (modelClass.isAssignableFrom(ProfileViewModel::class.java)) {
            val repo = ProfileRepositoryImpl(RetrofitClient.apiService)
